...
# go/blaze-colorizer
#
# Intercepts output of `blaze` to apply colors to log messages and test results.
# Replace errors with a more colorful version so they are easier to spot.
#
# Originally based on:
#   experimental/users/idryanovski/glog_color/glog_color.sh
#   experimental/users/rahm/sweethacks/blaze-colorme/blaze-colorme.sh
#
# Source this file (or place in ~/.bashrc or ~/.zshrc) and use blaze as usual:
#   blaze run -- "path/to/binary" --logtostderr
#   blaze build "//path/to:build"
#
# To revert this behavior:
#   Only for the next execution: `command blaze`.
#   For the entire shell session: `unset -f blaze`.
__blaze_color() {
  unbuffer "$@" 2>&1 | sed -u '
#!/bin/sed -f
...
iblaze() {
  if [ -t 1 ]; then
    __blaze_color iblaze --iblaze_interrupt_on_change "$@"
  else
    command iblaze "$@"
  fi
}
alias iiblaze="iblaze --iblaze_run_before clear"
# go/rabbit
rabbit() {
  if [ -t 1 ]; then
    __blaze_color rabbit "$@"
  else
    command rabbit "$@"
  fi
}
